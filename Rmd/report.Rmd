...
I defined significant differentially methylated promoters (DMPs) as those that showed both statistical significance (FDR p-value < $0.05$) and biological significant ($\Delta \beta > 0.05$). This resulted in `r sig(tissue)` tissue-DMPs, `r sig(age)` age-DMPs and `r sig(afr)` ancestry-DMPs. 

```{r echo=FALSE, fig.width=6, fig.height=6, fig.cap="Venn Diagram of differentially methylated promoters (DMPs)"}
...
We see the that ensemble model performs the better than any of the individual models in the ensemble. This is in concordance with literature of predictive modeling, where an ensemble of models typically outpreforms an individual model.Additionall we see that many of the commonly used methods, such as random forests (rf) and support vector machines with a radial kernel (svmRadialCost), show quite similar performances. 
...
To finalize the DMPs, we should continue to use both a biological and statistical criteria for significants. Different variations for the linear model used to measure the effect of aging, tissue group and ancestry can be used, and these variation can change the number of DMPs that are detected. 
...
The recalibrated epigenetic clock should be validated one ancestry and one tissue not included in the training set. For this, I suggest excluding the cerebellar samples, since they are outliers on the PCA plot, for the entirely analysis. Then, the cerebellar samples can be used as a testing set. Additionally, I would use pygmy samples to validate the clock performance on an ancestral group not included in the training set.
